import {getNewStats, exportNewStats} from '../apis/DataApi';
...
export function requestResetQueryParams() {
    return dispatch => {
        ...
        getJson('data/channels').then(res =>{
            const channel=res.map((item,index)=>{
                return {
                    value:item,
                    text:item,
                }
            })
            console.log('qudaohao:'+channel);
            dispatch({
...
export function requestExportList(page, pageSize, titles) {
    return (dispatch, getState) => {
        ...
        exportNewStats(params, titles).catch(err => {
